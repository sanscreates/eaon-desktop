...
// drives connect / list / call / disconnect through the four commands at the
// bottom.
...
pub struct McpCallResult {
    /// True when the TOOL reported failure (a successful RPC carrying an
    /// error result, per spec) — transport/protocol failures reject the
...
async fn http_rpc(
    ...
    let (endpoint, auth, session, initialized, request_id) = {
        let mut m = meta.lock().await;
        let id = m.next_id;
        if expects_reply {
            m.next_id += 1;
        }
        (m.endpoint.clone(), m.auth.clone(), m.session_id.clone(), m.initialized, id)
    };
    ...
    let client = crate::http_client(Some(HTTP_TIMEOUT_SECS));
...
fn call_result_from(result: &Value) -> McpCallResult {
...
/// connection under `request.id`, replacing any previous one.
#[tauri::command]
pub async fn mcp_connect(request: McpConnectRequest) -> Result<Vec<McpToolInfo>, String> {
    // Drop any prior connection for this id first (stdio children get
    // killed by StdioConn's Drop).
    CONNECTIONS.lock().unwrap().remove(&request.id);

    match request.transport.as_str() {
        "http" => {
            let url = request.url.clone().filter(|u| !u.trim().is_empty()).ok_or("This server needs a URL.")?;
            let token = request.token.clone().unwrap_or_default();
            let auth = if token.trim().is_empty() {
                None
            } else {
                Some((request.auth_scheme.clone().unwrap_or_else(|| "Bearer".to_string()), token.trim().to_string()))
            };
            ...
            CONNECTIONS.lock().unwrap().insert(request.id.clone(), Conn::Http(meta));
            Ok(tools)
        }
        "stdio" => {
            let command = request
            ...
            let tools = tauri::async_runtime::spawn_blocking(move || -> Result<(StdioConn, Vec<McpToolInfo>), String> {
                let mut conn = spawn_stdio(command.trim(), &args)?;
            ...
            CONNECTIONS
                .lock()
                .unwrap()
                .insert(request.id.clone(), Conn::Stdio(Arc::new(Mutex::new(conn))));
            Ok(list)
        }
...
#[tauri::command]
pub async fn mcp_call(id: String, tool: String, args: Value) -> Result<McpCallResult, String> {
    let conn = get_conn(&id).ok_or("Not connected yet.")?;
    ...
    match conn {
        ...
        Conn::Stdio(cell) => tauri::async_runtime::spawn_blocking(move || {
...
#[tauri::command]
pub fn mcp_disconnect(id: String) {
...
mod tests {
    ...
    fn tool_and_call_parsing_match_spec_shapes() {
        ...
        .unwrap();
        let result = call_result_from(&call);
        assert!(!result.is_error);
        assert_eq!(result.text, "hello\nworld");

        let empty: Value = serde_json::from_str(r#"{"content":[]}"#).unwrap();
        assert_eq!(call_result_from(&empty).text, "(no text content returned)");
    }
...
"#;
        ...
        .unwrap();
        assert_eq!(call_result_from(&result).text, "echo: hi");
    }
}
