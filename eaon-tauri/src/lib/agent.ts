...
export async function runTool(call: ToolCall): Promise<ToolOutcome> {
  ...
  try {
    return await invoke<ToolOutcome>("run_agent_tool", { name: call.name, args: call.args });
  } catch (e) {
...
export function agentInstruction(includeWiderTools: boolean): string {
...
- ask_user — ask the user ONE question and wait for their answer. Args: {"question": "...", "options": ["A", "B"]} (2–4 concrete options when the choices are known; they can always type their own answer instead).${widerToolLines}
